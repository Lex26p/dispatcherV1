...
На текущем этапе реализовано in-memory хранилище текущих значений тегов, применение результата чтения протокольного драйвера, обработка last good value, value conversion и engineering transform.
...
Метод без `Tag` сохраняет совместимость с предыдущим шагом:
...
## Почему метод без Tag остается

На ранних этапах polling может иметь только `ProtocolReadResult`.

Чтобы не ломать существующий код, оставлен метод:

    apply_protocol_read_result(read_result)

Он просто применяет значение без знания конфигурации тега.

Полный путь будет использовать метод:
...
- Добавляем базовое преобразование значений.
