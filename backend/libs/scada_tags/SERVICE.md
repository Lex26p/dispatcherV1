...
На текущем этапе добавлена только модель текущего значения и repository-интерфейс для текущих значений.
...
На текущем этапе добавлены repository-интерфейсы тегов.

### ITagRepository

Интерфейс хранения конфигурации тегов.
...
- поиск тега по `TagId`;
- поиск тега по `code`;
- получение тегов по `ObjectId`;
- получение тегов по `DeviceId`;
- получение тегов по `TagType`;
- получение тегов по `TagArchivePolicy`;
- сохранение тега;
- удаление тега.

### ITagCurrentValueRepository

Интерфейс доступа к текущим значениям тегов.
...
- очистку текущих значений.

Важно: `ITagCurrentValueRepository` пока является только интерфейсом.

Он не означает, что текущие значения обязательно будут храниться в PostgreSQL.
...
- PostgreSQL snapshot repository, если позже понадобится.
...
- `include/scada_tags/repository/ITagRepository.h`
- `include/scada_tags/repository/ITagCurrentValueRepository.h`
- `src/tag_type.cpp`
...
- Готовим модель тегов к будущему хранению и runtime-слою.
