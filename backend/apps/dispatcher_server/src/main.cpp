...
)
{
    for (
        auto module :
        dispatcher::core::get_initial_module_list()
    ) {
        auto result =
...
int main()
{
    ...
    const auto register_result =
        register_initial_modules(context);
    ...
    const auto configuration_read_service =
        dispatcher::app::
            create_development_configuration_read_service();

    auto http_route_dispatcher =
        dispatcher::http::
            make_system_route_dispatcher(
                context.modules()
            );

    const auto object_routes_registered =
        dispatcher::http::
    ...
    if (!object_routes_registered) {
        std::cerr
            << "Object HTTP routes failed "
               "to register."
            << '\n';
    ...
    dispatcher::http::DrogonHttpServer
        http_server{
            http_options,
            std::move(
                http_route_dispatcher
    ...
    const auto startup_time =
        dispatcher::common::now_utc();

    const auto startup_envelope =
    ...
    const auto startup_event =
        dispatcher::core::make_core_event(
            "ApplicationStarted",
            "Dispatcher server application "
            "context started",
            context
                .startup_correlation_id()
                .str()
    ...
    std::cout
        << "Press Enter to stop dispatcher_server..."
        << '\n';
